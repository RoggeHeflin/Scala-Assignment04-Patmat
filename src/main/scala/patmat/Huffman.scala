...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      ...
      (Map[Char, Int]() /: chars)(increment).toList
    }
   ...
   */
    def until(sng: List[CodeTree] => Boolean, comb: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if(sng(trees)) trees
      else until(sng, comb)(comb(trees))
    }
